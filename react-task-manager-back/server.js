  ...
  return new Promise((resolve) => {
    writeQueue.push(async () => {
      try {
        await fs.writeFile(
          DATA_FILE,
          JSON.stringify(tasksCache, null, 2),
          "utf-8"
        );
...
const validateTask = (task) => {
  ...
  if (
    ...
    errors.push(
      "Il campo 'title' è obbligatorio e deve essere una stringa non vuota."
    );
  }
  ...
  if (!validStatuses.includes(task.status)) {
    errors.push(
      `Il campo 'status' deve essere uno tra: ${validStatuses.join(", ")}.`
    );
  }
  return errors;
};
...
app.post("/tasks", async (req, res) => {
  ...
  newTask.id =
    tasksCache.length > 0 ? Math.max(...tasksCache.map((t) => t.id)) + 1 : 1;
...
app.put("/tasks/:id", async (req, res) => {
  ...
  if (taskIndex === -1) {
    return res
      .status(404)
      .json({ success: false, message: "Task non trovato." });
  }
...
if (!existsSync(DATA_FILE) || readFileSync(DATA_FILE, "utf-8").trim() === "") {
  console.log(
    "⚡ File tasks.json non trovato o vuoto, creazione con dati iniziali..."
  );
  ...
  tasksCache = [
    ...
    },
    {
      ...
      description: "Prenotare visita annuale",
      status: "To do",
      id: 3,
      createdAt: getRandomDateInLastWeek(),
    },
    {
      title: "Completare progetto React",
      description: "Task Manager avanzato",
      status: "Doing",
      id: 4,
      createdAt: getRandomDateInLastWeek(),
    },
    {
      title: "Scrivere articolo",
      description: "Nuovo post su blog tecnico",
      status: "Doing",
      id: 5,
      createdAt: getRandomDateInLastWeek(),
    },
    {
      ...
      description: "30 minuti di corsa",
      status: "Done",
      id: 6,
      createdAt: getRandomDateInLastWeek(),
    },
